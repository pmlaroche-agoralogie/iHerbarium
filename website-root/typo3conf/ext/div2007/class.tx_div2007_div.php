 ...
 *
 * $Id: class.t3lib_div.php 5713 2009-07-12 18:46:06Z flyguide $
...
final class tx_div2007_div {
	 ...
	 * This function substitutes tx_div2007_div::GPvar()
	 * To enhancement security in your scripts, please consider using tx_div2007_div::_GET or tx_div2007_div::_POST if you already know by which method your data is arriving to the scripts!
	 * Usage: 537
	 *
	...
	public static function _GP($var)	{
		...
		if (isset($value))	{
			if (is_array($value))	{ tx_div2007_div::stripSlashesOnArray($value); } else { $value = stripslashes($value); }
		}
		return $value;
	}
	...
	public static function _GPmerged($parameter) {
		...
		$mergedParameters = tx_div2007_div::array_merge_recursive_overrule($getParameter, $postParameter);
		tx_div2007_div::stripSlashesOnArray($mergedParameters);
	...
	public static function _GETset($inputGet,$key='')	{
		...
		if (strcmp($key,''))	{
			if (is_array($inputGet)) {
				tx_div2007_div::addSlashesOnArray($inputGet);
			} else {
	 ...
	 * @return	mixed		POST var named $var and if not set, the GET var of the same name.
	 * @deprecated since TYPO3 3.6 - Use tx_div2007_div::_GP instead (ALWAYS delivers a value with un-escaped values!)
	 * @see _GP()
	 */
	public static function GPvar($var,$strip=0)	{
		...
		if ($strip && isset($value) && is_array($value)) { tx_div2007_div::stripSlashesOnArray($value); }
		return $value;
	}
	 ...
	 * @param	string		Key (variable name) from GET or POST vars
	 * @return	array		Returns the GET vars merged recursively onto the POST vars.
	 * @deprecated since TYPO3 3.7 - Use tx_div2007_div::_GPmerged instead
	 * @see _GP()
	 */
	...
	public static function gif_compress($theFile, $type)	{
		...
		if ($gfxConf['gif_compress'] && strtolower(substr($theFile,-4,4))=='.gif')	{	// GIF...
			if (($type=='IM' || !$type) && $gfxConf['im'] && $gfxConf['im_path_lzw'])	{	// IM
				$cmd = tx_div2007_div::imageMagickCommand('convert', '"'.$theFile.'" "'.$theFile.'"', $gfxConf['im_path_lzw']);
				exec($cmd);
	...
	public static function png_to_gif_by_imagemagick($theFile)	{
		if ($GLOBALS['TYPO3_CONF_VARS']['FE']['png_to_gif']
			...
			&& @is_file($theFile))	{	// IM
				...
				$cmd = tx_div2007_div::imageMagickCommand('convert', '"'.$theFile.'" "'.$newFile.'"', $GLOBALS['TYPO3_CONF_VARS']['GFX']['im_path_lzw']);
				exec($cmd);
				$theFile = $newFile;
	...
	public static function read_png_gif($theFile,$output_png=0)	{
		if ($GLOBALS['TYPO3_CONF_VARS']['GFX']['im'] && @is_file($theFile))	{
			...
			if (
				...
				$cmd = tx_div2007_div::imageMagickCommand('convert', '"'.$theFile.'" "'.$newFile.'"', $GLOBALS['TYPO3_CONF_VARS']['GFX']['im_path']);
				exec($cmd);
	 ...
	 * @deprecated since TYPO3 4.1 - Works ONLY for single-byte charsets! Use tx_div2007_div::fixed_lgd_cs() instead
	 * @see fixed_lgd_pre()
	 */
	...
	public static function fixed_lgd_pre($string,$chars)	{
		...
		return strrev(tx_div2007_div::fixed_lgd(strrev($string),$chars));
	}
	...
	public static function fixed_lgd_cs($string, $chars, $appendString='...') {
		if (is_object($GLOBALS['LANG'])) {
			...
			$csConvObj = tx_div2007_div::makeInstance('t3lib_cs');
	...
	public static function breakTextForEmail($str,$implChar="\n",$charWidth=76)	{
		...
		foreach ($lines as $lStr) {
			$outArr[] = tx_div2007_div::breakLinesForEmail($lStr,$implChar,$charWidth);
		}
	...
	public static function cmpIP($baseIP, $list)	{
		...
		if (strpos($baseIP, ':') !== false && tx_div2007_div::validIPv6($baseIP))	{
			return tx_div2007_div::cmpIPv6($baseIP, $list);
		} else {
			return tx_div2007_div::cmpIPv4($baseIP, $list);
		}
	}
	...
	public static function cmpIPv4($baseIP, $list)	{
		...
		if (count($IPpartsReq)==4)	{
			$values = tx_div2007_div::trimExplode(',',$list,1);
	...
	public static function cmpIPv6($baseIP, $list)	{
		...
		$baseIP = tx_div2007_div::normalizeIPv6($baseIP);

		$values = tx_div2007_div::trimExplode(',',$list,1);
		foreach ($values as $test)	{
			list($test,$mask) = explode('/',$test);
			if (tx_div2007_div::validIPv6($test))	{
				$test = tx_div2007_div::normalizeIPv6($test);
				if (intval($mask))	{
					switch ($mask) {	// test on /48 /64
						case '48':
							$testBin = substr(tx_div2007_div::IPv6Hex2Bin($test), 0, 48);
							$baseIPBin = substr(tx_div2007_div::IPv6Hex2Bin($baseIP), 0, 48);
							$success = strcmp($testBin, $baseIPBin)==0 ? true : false;
						break;
						case '64':
							$testBin = substr(tx_div2007_div::IPv6Hex2Bin($test), 0, 64);
							$baseIPBin = substr(tx_div2007_div::IPv6Hex2Bin($baseIP), 0, 64);
							$success = strcmp($testBin, $baseIPBin)==0 ? true : false;
						break;
						default:
							$success = false;
					...
					if (tx_div2007_div::validIPv6($test))	{	// test on full ip address 128 bits
						$testBin = tx_div2007_div::IPv6Hex2Bin($test);
						$baseIPBin = tx_div2007_div::IPv6Hex2Bin($baseIP);
						$success = strcmp($testBin, $baseIPBin)==0 ? true : false;
					}
				}
			}
	...
	public static function compat_version($verNumberStr)	{
		...
		if (tx_div2007_div::int_from_ver($currVersionStr) < tx_div2007_div::int_from_ver($verNumberStr))	{
			return FALSE;
		} else {
			return TRUE;
		}
	...
	public static function uniqueList($in_list, $secondParameter=NULL)	{
		if (is_array($in_list))	die('tx_div2007_div::uniqueList() does NOT support array arguments anymore! Only string comma lists!');
		if (isset($secondParameter))	die('tx_div2007_div::uniqueList() does NOT support more than a single argument value anymore. You have specified more than one.');

		return implode(',',array_unique(tx_div2007_div::trimExplode(',',$in_list,1)));
	}
	...
	public static function dirname($path)	{
		$p = tx_div2007_div::revExplode('/',$path,2);
	...
	public static function modifyHTMLColor($color,$R,$G,$B)	{
		...
		$nR = tx_div2007_div::intInRange(hexdec(substr($color,1,2))+$R,0,255);
		$nG = tx_div2007_div::intInRange(hexdec(substr($color,3,2))+$G,0,255);
		$nB = tx_div2007_div::intInRange(hexdec(substr($color,5,2))+$B,0,255);
		return '#'.
	...
	public static function modifyHTMLColorAll($color,$all)	{
		return tx_div2007_div::modifyHTMLColor($color,$all,$all,$all);
	}
	...
	public static function calcParenthesis($string)	{
		...
		do {
			...
			if ($valueLenC==strlen($string) || $valueLenC < $valueLenO)	{
				$value = tx_div2007_div::calcPriority(substr($string,0,$valueLenC));
				$string = $value.substr($string,$valueLenC+1);
				return $string;
			} else {
				$string = substr($string,0,$valueLenO).tx_div2007_div::calcParenthesis(substr($string,$valueLenO+1));
			}
				// Security:
			$securC--;
			if ($securC<=0)	break;
	...
	public static function removeArrayEntryByValue(array $array, $cmpValue)	{
		foreach ($array as $k => $v) {
			if (is_array($v)) {
				$array[$k] = tx_div2007_div::removeArrayEntryByValue($v, $cmpValue);
	...
	public static function keepItemsInArray(array $array, $keepItems, $getValueFunc=null) {
		if ($array) {
			...
			if (is_string($keepItems)) {
				$keepItems = tx_div2007_div::trimExplode(',', $keepItems);
			}
	...
	public static function implodeArrayForUrl($name, array $theArray, $str = '', $skipBlank = false, $rawurlencodeParamName = false) {
		foreach($theArray as $Akey => $AVal)	{
			...
			if (is_array($AVal))	{
				$str = tx_div2007_div::implodeArrayForUrl($thisKeyName,$AVal,$str,$skipBlank,$rawurlencodeParamName);
			} else {
	 ...
	 * @param	boolean		If set, then tx_div2007_div::_GP() is used to fetch the value if not found (isset) in the $getArray
	...
	public static function compileSelectedGetVarsFromArray($varList,array $getArray,$GPvarAlt=1)	{
		$keys = tx_div2007_div::trimExplode(',',$varList,1);
		$outArr = array();
		foreach($keys as $v)	{
			if (isset($getArray[$v]))	{
				$outArr[$v] = $getArray[$v];
			} elseif ($GPvarAlt) {
				$outArr[$v] = tx_div2007_div::_GP($v);
			}
		}
		return $outArr;
	}
	...
	public static function addSlashesOnArray(array &$theArray)	{
		foreach ($theArray as &$value) {
			if (is_array($value)) {
				tx_div2007_div::addSlashesOnArray($value);
			} else {
	...
	public static function stripSlashesOnArray(array &$theArray)	{
		foreach ($theArray as &$value) {
			if (is_array($value)) {
				tx_div2007_div::stripSlashesOnArray($value);
			} else {
	...
	public static function slashArray(array $arr,$cmd)	{
		if ($cmd=='strip')	tx_div2007_div::stripSlashesOnArray($arr);
		if ($cmd=='add')	tx_div2007_div::addSlashesOnArray($arr);
		return $arr;
	}
		...
		foreach ($arr1 as $key => $val) {
			if(is_array($arr0[$key])) {
				if (is_array($arr1[$key]))	{
					$arr0[$key] = tx_div2007_div::array_merge_recursive_overrule($arr0[$key],$arr1[$key],$notAddKeys,$includeEmtpyValues);
				}
			} else {
				if ($notAddKeys) {
					if (isset($arr0[$key])) {
	...
	public static function removeDotsFromTS(array $ts) {
		...
		foreach ($ts as $key => $value) {
			if (is_array($value)) {
				$key = rtrim($key, '.');
				$out[$key] = tx_div2007_div::removeDotsFromTS($value);
			} else {
				$out[$key] = $value;
			}
		}
	...
	public static function get_tag_attributes($tag)	{
		$components = tx_div2007_div::split_tag_attributes($tag);
	...
	public static function implodeParams(array $arr,$xhtmlSafe=FALSE,$dontOmitBlankAttribs=FALSE)	{
		...
		return tx_div2007_div::implodeAttributes($arr,$xhtmlSafe,$dontOmitBlankAttribs);
	}
...
// FIXME don't use unset() - what does that mean? Use NULL or similar.
			...
			if ($type=='complete' || $type=='close')	{
				...
				if ($depth==($stacktop+1))	{
					if ($key-$startPoint > 0)	{
						...
						#$oldtagi=array('XMLvalue'=>tx_div2007_div::xmlRecompileFromStructValArray($partArray));
						$oldtagi['XMLvalue']=tx_div2007_div::xmlRecompileFromStructValArray($partArray);
					} else {
	...
	public static function array2xml_cs(array $array,$docTag='phparray',array $options=array(),$charset='')	{
		...
		return '<?xml version="1.0" encoding="'.htmlspecialchars($charset).'" standalone="yes" ?>'.chr(10).
				tx_div2007_div::array2xml($array,'',0,$docTag,0, $options);
	}
	...
	public static function array2xml(array $array,$NSprefix='',$level=0,$docTag='phparray',$spaceInd=0,array $options=array(),array $stackData=array())	{
		...
		foreach($array as $k=>$v)	{
			...
			}elseif(isset($options['parentTagMap'][$stackData['parentTagName'].':_IS_NUM']) && tx_div2007_div::testInt($tagName)) {		// Use tag based on parent tag name + if current tag is numeric
				$attr.=' index="'.htmlspecialchars($tagName).'"';
...
// FHO end
	...
	public static function xmlGetHeaderAttribs($xmlData)	{
		...
		if (preg_match('/^\s*<\?xml([^>]*)\?\>/', $xmlData, $match))	{
			return tx_div2007_div::get_tag_attributes($match[1]);
		}
	}
	...
	public static function minifyJavaScript($script, &$error = '') {
		...
		try {
			...
			tx_div2007_div::devLog($error, 'tx_div2007_div', 2,
	...
	public static function writeFile($file,$content)	{
		...
		if ($fd = fopen($file,'wb'))	{
			...
			if ($changePermissions)	{	// Change the permissions only if the file has just been created
				tx_div2007_div::fixPermissions($file);
			}
	...
	public static function writeFileToTypo3tempDir($filepath,$content)	{
		...
		$fI['dirname'].= '/';
			...
			// Check parts:
		if (tx_div2007_div::validPathStr($filepath) && $fI['basename'] && strlen($fI['basename'])<60)	{
			if (defined('PATH_site'))	{
				$dirName = PATH_site.'typo3temp/';	// Setting main temporary directory name (standard)
				if (@is_dir($dirName))	{
					if (tx_div2007_div::isFirstPartOfStr($fI['dirname'],$dirName))	{
						...
						if ($subdir)	{
							if (preg_match('/^[[:alnum:]_]+\/$/',$subdir) || preg_match('/^[[:alnum:]_]+\/[[:alnum:]_]+\/$/',$subdir))	{
								$dirName.= $subdir;
								if (!@is_dir($dirName))	{
									tx_div2007_div::mkdir_deep(PATH_site.'typo3temp/', $subdir);
								}
						...
						if (@is_dir($dirName))	{
							if ($filepath == $dirName.$fI['basename'])	{
								tx_div2007_div::writeFile($filepath, $content);
	...
	public static function mkdir_deep($destination,$deepDir)	{
		$allParts = tx_div2007_div::trimExplode('/',$deepDir,1);
		$root = '';
		foreach($allParts as $part)	{
			$root.= $part.'/';
			if (!is_dir($destination.$root))	{
				tx_div2007_div::mkdir($destination.$root);
				if (!@is_dir($destination.$root))	{
			...
			if (is_dir($path))	{
				if ($removeNonEmpty==true && $handle = opendir($path))	{
					while ($OK && false !== ($file = readdir($handle)))	{
						...
						$OK = tx_div2007_div::rmdir($path.'/'.$file,$removeNonEmpty);
					}
					closedir($handle);
				}
				if ($OK)	{ $OK = rmdir($path); }
	 ...
	 * @param	array		Array of read in types with keys 'file', 'dir'
	 * @return	array		Array of the files found
	 */
	public static function getFilesInDir($path,$extensionList='',$prependPath=0,$order='',$excludePattern='',$typeArray=array())	{

		if (!count($typeArray))	{
			$typeArray['file'] = TRUE; // reading of files is activated by default.
		}
		...
		if (@is_dir($path))	{
			...
			if (is_object($d))	{
				...
				while($entry=$d->read()) {
					if (
						$typeArray['file'] && @is_file($path.'/'.$entry) ||
						$typeArray['dir'] && @is_dir($path.'/'.$entry) && $entry != '.' && $entry != '..'
					)	{
						$fI = pathinfo($entry);
						if ((!strlen($extensionList) || tx_div2007_div::inList($extensionList,strtolower($fI['extension']))) && (!strlen($excludePattern) || !preg_match('/^'.$excludePattern.'$/',$entry)))	{
	...
	public static function getAllFilesAndFoldersInPath(array $fileArr,$path,$extList='',$regDirs=0,$recursivityLevels=99,$excludePattern='')	{
		...
		$fileArr = array_merge($fileArr, tx_div2007_div::getFilesInDir($path,$extList,1,1,$excludePattern));

		$dirs = tx_div2007_div::get_dirs($path);
		if (is_array($dirs) && $recursivityLevels>0)	{
			foreach ($dirs as $subdirs)	{
				if ((string)$subdirs!='' && (!strlen($excludePattern) || !preg_match('/^'.$excludePattern.'$/',$subdirs)))	{
					$fileArr = tx_div2007_div::getAllFilesAndFoldersInPath($fileArr,$path.$subdirs.'/',$extList,$regDirs,$recursivityLevels-1,$excludePattern);
				}
			}
		}
		return $fileArr;
	...
	public static function removePrefixPathFromList(array $fileArr,$prefixToRemove)	{
		foreach ($fileArr as $k => &$absFileRef) {
			if (tx_div2007_div::isFirstPartOfStr($absFileRef, $prefixToRemove)) {
	...
	public static function locationHeaderUrl($path)	{
		...
		if (substr($path,0,1)=='/')	{ // relative to HOST
			$path = tx_div2007_div::getIndpEnv('TYPO3_REQUEST_HOST').$path;
		} elseif (!$uI['scheme'])	{ // No scheme either
			$path = tx_div2007_div::getIndpEnv('TYPO3_REQUEST_DIR').$path;
		}
		return $path;
	}
	 ...
	 * use tx_div2007_div::print_array() in order to print an array
			...
			if (count($array_in) == 0)	{
				...
				foreach ($array_in as $key => $val)	{
					$result.= '<tr>
						...
						<td>';
					if (is_array($val))	{
						$result.=tx_div2007_div::view_array($val);
	...
	public static function print_array($array_in)	{
		echo tx_div2007_div::view_array($array_in);
	}
	 ...
	 * If $var is an array, the array is printed by tx_div2007_div::print_array()
	 * Usage: 8
	 *
	...
	public static function debug($var='',$brOrHeader=0)	{
		...
		if ($brOrHeader && !tx_div2007_div::testInt($brOrHeader))	{
		...
		if (is_array($var))	{
			tx_div2007_div::print_array($var);
		...
		if (is_array($rows))	{
			...
			if (is_array($firstEl))	{
				...
				foreach($rows as $singleRow)	{
					...
					foreach($headerColumns as $key)	{
						$tCells[] = '
							<td><font face="Verdana,Arial" size="1">'.(is_array($singleRow[$key]) ? tx_div2007_div::debugRows($singleRow[$key],'',TRUE) : htmlspecialchars($singleRow[$key])).'</font></td>';
					}
					$tRows[] = '
						<tr>'.implode('',$tCells).'
						</tr>';
	...
	public static function getThisUrl()	{
		$p=parse_url(tx_div2007_div::getIndpEnv('TYPO3_REQUEST_SCRIPT'));		// Url of this script
		$dir=tx_div2007_div::dirname($p['path']).'/';	// Strip file
	...
	public static function linkThisScript(array $getParams = array()) {
		$parts = tx_div2007_div::getIndpEnv('SCRIPT_NAME');
		$params = tx_div2007_div::_GET();
		...
		$pString = tx_div2007_div::implodeArrayForUrl('', $params);
	...
	public static function linkThisUrl($url,array $getParams=array())	{
		...
		$getP = tx_div2007_div::array_merge_recursive_overrule($getP,$getParams);
		$uP = explode('?',$url);

		$params = tx_div2007_div::implodeArrayForUrl('',$getP);
	 ...
	 */
	public static function redirect($url, $httpStatus = tx_div2007_div::HTTP_STATUS_303) {
		header($httpStatus);
		header('Location: ' . tx_div2007_div::locationHeaderUrl($url));
...
#		if ($getEnvName=='HTTP_REFERER')	return '';
		...
		switch ((string)$getEnvName)	{
			case 'SCRIPT_NAME':
				...
				if (tx_div2007_div::cmpIP($_SERVER['REMOTE_ADDR'], $GLOBALS['TYPO3_CONF_VARS']['SYS']['reverseProxyIP'])) {
					if (tx_div2007_div::getIndpEnv('TYPO3_SSL') && $GLOBALS['TYPO3_CONF_VARS']['SYS']['reverseProxyPrefixSSL']) {
			...
			case 'REQUEST_URI':
				...
				if ($GLOBALS['TYPO3_CONF_VARS']['SYS']['requestURIvar'])	{	// This is for URL rewriters that store the original URI in a server variable (eg ISAPI_Rewriter for IIS: HTTP_X_REWRITE_URL)
					...
					$retVal = '/'.ltrim(tx_div2007_div::getIndpEnv('SCRIPT_NAME'), '/').
...
//				if (strcmp($_SERVER['PATH_INFO'],tx_div2007_div::getIndpEnv('SCRIPT_NAME')) && count(explode('/',$_SERVER['PATH_INFO']))>1)	{
				if (PHP_SAPI!='cgi' && PHP_SAPI!='cgi-fcgi')	{
					$retVal = $_SERVER['PATH_INFO'];
				}
			break;
			case 'TYPO3_REV_PROXY':
				$retVal = tx_div2007_div::cmpIP($_SERVER['REMOTE_ADDR'], $GLOBALS['TYPO3_CONF_VARS']['SYS']['reverseProxyIP']);
			break;
			case 'REMOTE_ADDR':
				$retVal = $_SERVER['REMOTE_ADDR'];
				if (tx_div2007_div::cmpIP($_SERVER['REMOTE_ADDR'], $GLOBALS['TYPO3_CONF_VARS']['SYS']['reverseProxyIP'])) {
					$ip = tx_div2007_div::trimExplode(',', $_SERVER['HTTP_X_FORWARDED_FOR']);
			...
			case 'HTTP_HOST':
				...
				if (tx_div2007_div::cmpIP($_SERVER['REMOTE_ADDR'], $GLOBALS['TYPO3_CONF_VARS']['SYS']['reverseProxyIP'])) {
					$host = tx_div2007_div::trimExplode(',', $_SERVER['HTTP_X_FORWARDED_HOST']);
			...
			case 'TYPO3_DOCUMENT_ROOT':
				...
				$SFN = tx_div2007_div::getIndpEnv('SCRIPT_FILENAME');
				$SN_A = explode('/',strrev(tx_div2007_div::getIndpEnv('SCRIPT_NAME')));
			...
			case 'TYPO3_HOST_ONLY':
				$p = explode(':',tx_div2007_div::getIndpEnv('HTTP_HOST'));
				$retVal = $p[0];
			break;
			case 'TYPO3_PORT':
				$p = explode(':',tx_div2007_div::getIndpEnv('HTTP_HOST'));
			...
			case 'TYPO3_REQUEST_HOST':
				$retVal = (tx_div2007_div::getIndpEnv('TYPO3_SSL') ? 'https://' : 'http://').
					tx_div2007_div::getIndpEnv('HTTP_HOST');
			break;
			case 'TYPO3_REQUEST_URL':
				$retVal = tx_div2007_div::getIndpEnv('TYPO3_REQUEST_HOST').tx_div2007_div::getIndpEnv('REQUEST_URI');
			break;
			case 'TYPO3_REQUEST_SCRIPT':
				$retVal = tx_div2007_div::getIndpEnv('TYPO3_REQUEST_HOST').tx_div2007_div::getIndpEnv('SCRIPT_NAME');
			break;
			case 'TYPO3_REQUEST_DIR':
				$retVal = tx_div2007_div::getIndpEnv('TYPO3_REQUEST_HOST').tx_div2007_div::dirname(tx_div2007_div::getIndpEnv('SCRIPT_NAME')).'/';
			break;
			case 'TYPO3_SITE_URL':
				if (defined('PATH_thisScript') && defined('PATH_site'))	{
					$lPath = substr(dirname(PATH_thisScript),strlen(PATH_site)).'/';
					$url = tx_div2007_div::getIndpEnv('TYPO3_REQUEST_DIR');
			...
			case 'TYPO3_SITE_PATH':
				$retVal = substr(tx_div2007_div::getIndpEnv('TYPO3_SITE_URL'), strlen(tx_div2007_div::getIndpEnv('TYPO3_REQUEST_HOST')));
			break;
			case 'TYPO3_SITE_SCRIPT':
				$retVal = substr(tx_div2007_div::getIndpEnv('TYPO3_REQUEST_URL'),strlen(tx_div2007_div::getIndpEnv('TYPO3_SITE_URL')));
			break;
			case 'TYPO3_SSL':
				...
				if (tx_div2007_div::cmpIP($_SERVER['REMOTE_ADDR'], $proxySSL))	{
					$retVal = true;
				} else {
			...
			case '_ARRAY':
				...
				$envTestVars = tx_div2007_div::trimExplode(',','
					HTTP_HOST,
					TYPO3_HOST_ONLY,
					TYPO3_PORT,
					PATH_INFO,
				...
				foreach ($envTestVars as $v) {
					$out[$v]=tx_div2007_div::getIndpEnv($v);
				}
				reset($out);
				$retVal = $out;
			break;
	...
	/**
	 * milliseconds
	 * microtime recalculated to tx_div2007_div::milliseconds(1/1000 sec)
	 * Usage: 20
	 *
	 * @return	integer
	 */
	 ...
	 * Usage: 4
	 *
	 * @param	string		Alternative User Agent string (if empty, tx_div2007_div::getIndpEnv('HTTP_USER_AGENT') is used)
	...
	public static function clientInfo($useragent='')	{
		if (!$useragent) $useragent=tx_div2007_div::getIndpEnv('HTTP_USER_AGENT');
	...
	public static function getHostname($requestHost=TRUE)	{
		...
		if ($requestHost && (!defined('TYPO3_cliMode') || !TYPO3_cliMode))	{
			$host = tx_div2007_div::getIndpEnv('HTTP_HOST');
		}
		if (!$host)	{
	...
	public static function getFileAbsFileName($filename,$onlyRelative=TRUE,$relToTYPO3_mainDir=FALSE)	{
		...
		} elseif (!tx_div2007_div::isAbsPath($filename))	{	// relative. Prepended with $relPathPrefix
			$filename=$relPathPrefix.$filename;
		} elseif ($onlyRelative && !tx_div2007_div::isFirstPartOfStr($filename,$relPathPrefix)) {	// absolute, but set to blank if not allowed
			$filename='';
		}
		if (strcmp($filename,'') && tx_div2007_div::validPathStr($filename))	{	// checks backpath.
			return $filename;
		}
	}
	...
	public static function isAllowedAbsPath($path)	{
		if (tx_div2007_div::isAbsPath($path) &&
			tx_div2007_div::validPathStr($path) &&
				(	tx_div2007_div::isFirstPartOfStr($path,PATH_site)
					||
					($GLOBALS['TYPO3_CONF_VARS']['BE']['lockRootPath'] && tx_div2007_div::isFirstPartOfStr($path,$GLOBALS['TYPO3_CONF_VARS']['BE']['lockRootPath']))
				)
			)	return true;
	}
	...
	public static function upload_copy_move($source,$destination)	{
		...
		tx_div2007_div::fixPermissions($destination);	// Change the permissions of the file
	...
	public static function upload_to_tempfile($uploadedFileName)	{
		if (is_uploaded_file($uploadedFileName))	{
			$tempFile = tx_div2007_div::tempnam('upload_temp_');
	...
	public static function unlink_tempfile($uploadedTempFileName)	{
		if ($uploadedTempFileName && tx_div2007_div::validPathStr($uploadedTempFileName) && tx_div2007_div::isFirstPartOfStr($uploadedTempFileName,PATH_site.'typo3temp/') && @is_file($uploadedTempFileName))	{
	...
	public static function stdAuthCode($uid_or_record,$fields='',$codeLength=8)	{
		...
		if (is_array($uid_or_record))	{
			$recCopy_temp=array();
			if ($fields)	{
				$fieldArr = tx_div2007_div::trimExplode(',',$fields,1);
	...
	public static function cHashParams($addQueryParams) {
		...
		foreach($params as $theP)	{
			...
			if (!tx_div2007_div::inList('id,type,no_cache,cHash,MP,ftu',$pKV[0]) && !preg_match('/TSFE_ADMIN_PANEL\[.*?\]/',$pKV[0]))	{
		...
		if (isset($GLOBALS['TYPO3_CONF_VARS']['SC_OPTIONS']['t3lib/class.tx_div2007_div.php']['cHashParamsHook']))	{
			$cHashParamsHook =& $GLOBALS['TYPO3_CONF_VARS']['SC_OPTIONS']['t3lib/class.tx_div2007_div.php']['cHashParamsHook'];
			if (is_array($cHashParamsHook)) {
				...
				foreach ($cHashParamsHook as $hookFunction)	{
					tx_div2007_div::callUserFunction($hookFunction, $hookParameters, $hookReference);
				}
			}
		}
	 ...
	 *
	 * @param	string		Input is a file-reference (see tx_div2007_div::getFileAbsFileName). That file is expected to be a 'locallang.php' file containing a $LOCAL_LANG array (will be included!) or a 'locallang.xml' file conataining a valid XML TYPO3 language structure.
	...
	public static function readLLfile($fileRef, $langKey, $charset='')	{
		...
		$file = tx_div2007_div::getFileAbsFileName($fileRef);
		if ($file)	{
			$baseFile = preg_replace('/\.(php|xml)$/', '', $file);

			if (@is_file($baseFile.'.xml')) {
				$LOCAL_LANG = tx_div2007_div::readLLXMLfile($baseFile.'.xml', $langKey, $charset);
			} elseif (@is_file($baseFile.'.php'))   {
				if ($GLOBALS['TYPO3_CONF_VARS']['BE']['forceCharset'] || $charset)  {
					$LOCAL_LANG = tx_div2007_div::readLLPHPfile($baseFile.'.php', $langKey, $charset);
				} else {
	...
	public static function readLLPHPfile($fileRef, $langKey, $charset='')	{
		...
		if (@is_file($fileRef) && $langKey)	{
			...
			$cacheFileName = PATH_site.'typo3temp/llxml/'.
							...
							'_'.tx_div2007_div::shortMD5($hashSource).'.'.$langKey.'.'.$targetCharset.'.cache';
			...
			if (!@is_file($cacheFileName))	{	// ... if it doesn't, create content and write it:
				...
				$res = tx_div2007_div::writeFileToTypo3tempDir($cacheFileName, serialize($serContent));
				if ($res)	die('ERROR: '.$res);
			} else {
					// Get content from cache:
				$serContent = unserialize(tx_div2007_div::getUrl($cacheFileName));
	...
	public static function readLLXMLfile($fileRef, $langKey, $charset='')	{
		...
		if (@is_file($fileRef) && $langKey)	{
			...
			if (!@is_file($cacheFileName))	{	// ... if it doesn't, create content and write it:
					...
					// Read XML, parse it.
				$xmlString = tx_div2007_div::getUrl($fileRef);
				$xmlContent = tx_div2007_div::xml2array($xmlString);
				...
				if ($langKey!='default')	{
					...
					$LOCAL_LANG[$langKey] = tx_div2007_div::llXmlAutoFileName($fileRef, $langKey);
					$localized_file = tx_div2007_div::getFileAbsFileName($LOCAL_LANG[$langKey]);
			...
			if ($langKey!='default' && is_string($LOCAL_LANG[$langKey]) && strlen($LOCAL_LANG[$langKey]))	{
				...
				if ($localized_file && @is_file($localized_file))	{
					...
					if (!@is_file($cacheFileName))	{	// ... if it doesn't, create content and write it:
						...
						$local_xmlString = tx_div2007_div::getUrl($localized_file);
						$local_xmlContent = tx_div2007_div::xml2array($local_xmlString);
	...
	public static function llXmlAutoFileName($fileRef,$language)	{
		...
		if (tx_div2007_div::isFirstPartOfStr($fileRef,PATH_typo3.'sysext/'))	{	// Is system:
		...
		} elseif (tx_div2007_div::isFirstPartOfStr($fileRef,PATH_typo3.'ext/'))	{	// Is global:
			$validatedPrefix = PATH_typo3.'ext/';
		} elseif (tx_div2007_div::isFirstPartOfStr($fileRef,PATH_typo3conf.'ext/'))	{	// Is local:
		...
		if ($validatedPrefix)	{
			...
			$temp = tx_div2007_div::revExplode('/',$file_extPath,2);
		...
		if (isset($TCA[$table])) {
			...
			if (!$tca['columns']) {
				...
				if ($dcf) {
					...
					} elseif (tx_div2007_div::isAbsPath($dcf) && @is_file($dcf))	{	// Absolute path...
						include($dcf);
	...
	public static function resolveSheetDefInDS($dataStructArray,$sheet='sDEF')	{
		...
		if (is_array($dataStructArray['sheets']))	{
			...
			if ($dataStruct && !is_array($dataStruct))	{
				$file = tx_div2007_div::getFileAbsFileName($dataStruct);
				if ($file && @is_file($file))	{
					$dataStruct = tx_div2007_div::xml2array(tx_div2007_div::getUrl($file));
				}
			}
		} else {
			$singleSheet = TRUE;
	...
	public static function resolveAllSheetsInDS(array $dataStructArray)	{
		if (is_array($dataStructArray['sheets']))	{
			...
			foreach($dataStructArray['sheets'] as $sheetId => $sDat)	{
				list($ds,$aS) = tx_div2007_div::resolveSheetDefInDS($dataStructArray,$sheetId);
				if ($sheetId==$aS)	{
					$out['sheets'][$aS]=$ds;
				}
			}
		} else {
			list($ds) = tx_div2007_div::resolveSheetDefInDS($dataStructArray);
	...
	public static function callUserFunction($funcName,&$params,&$ref,$checkPrefix='user_',$errorMode=0)	{
		...
		if (strpos($funcName,':') !== false)	{
			list($file,$funcRef) = tx_div2007_div::revExplode(':',$funcName,2);
			$requireFile = tx_div2007_div::getFileAbsFileName($file);
			if ($requireFile) tx_div2007_div::requireOnce($requireFile);
		} else {
			$funcRef = $funcName;
		}
		...
		if ($checkPrefix &&
			!tx_div2007_div::isFirstPartOfStr(trim($funcRef),$checkPrefix) &&
			!tx_div2007_div::isFirstPartOfStr(trim($funcRef),'tx_')
			)	{
			...
			if (class_exists($parts[0]))	{
				...
				if ($storePersistentObject)	{	// Get reference to current instance of class:
					if (!is_object($GLOBALS['T3_VAR']['callUserFunction_classPool'][$parts[0]]))	{
						$GLOBALS['T3_VAR']['callUserFunction_classPool'][$parts[0]] = &tx_div2007_div::makeInstance($parts[0]);
					}
					$classObj = &$GLOBALS['T3_VAR']['callUserFunction_classPool'][$parts[0]];
				} else {	// Create new object:
					$classObj = &tx_div2007_div::makeInstance($parts[0]);
				}
	...
	public static function &getUserObj($classRef,$checkPrefix='user_',$silent=0)	{
		...
		if (is_object($GLOBALS['T3_VAR']['getUserObj'][$classRef]))	{
			...
			if (strpos($classRef,':') !== false)	{
				list($file,$class) = tx_div2007_div::revExplode(':',$classRef,2);
				$requireFile = tx_div2007_div::getFileAbsFileName($file);
				if ($requireFile)	tx_div2007_div::requireOnce($requireFile);
			} else {
				$class = $classRef;
			...
			if (substr($class,0,1)=='&')	{
				...
				// Check prefix is valid:
			if ($checkPrefix &&
				!tx_div2007_div::isFirstPartOfStr(trim($class),$checkPrefix) &&
				!tx_div2007_div::isFirstPartOfStr(trim($class),'tx_')
				)	{
			...
			if (class_exists($class))	{
				$classObj = &tx_div2007_div::makeInstance($class);
	 ...
	 * @deprecated since TYPO3 4.3 - Use tx_div2007_div::makeInstance('myClass', $arg1, $arg2,  ..., $argN)
	 */
	public static function makeInstanceClassName($className)	{
		self::logDeprecatedFunction();

		return (class_exists($className) && class_exists('ux_'.$className, false) ? tx_div2007_div::makeInstanceClassName('ux_' . $className) : $className);
	}
	...
	public static function &makeInstanceService($serviceType, $serviceSubType='', $excludeServiceKeys=array())	{
		...
		if (!is_array($excludeServiceKeys) ) {
			$excludeServiceKeys = tx_div2007_div::trimExplode(',', $excludeServiceKeys, 1);
		}
		while ($info = t3lib_extMgm::findService($serviceType, $serviceSubType, $excludeServiceKeys))	{
			...
			if (is_object($GLOBALS['T3_VAR']['makeInstanceService'][$info['className']]))	{
				...
				$requireFile = tx_div2007_div::getFileAbsFileName($info['classFile']);
				if (@is_file($requireFile)) {
					tx_div2007_div::requireOnce ($requireFile);
					$obj = tx_div2007_div::makeInstance($info['className']);
					if (is_object($obj)) {
						if(!@is_callable(array($obj,'init')))	{
								// use silent logging??? I don't think so.
							die ('Broken service:'.tx_div2007_div::view_array($info));
						}
						$obj->info = $info;
	...
	public static function plainMailEncoded($email,$subject,$message,$headers='',$encoding='quoted-printable',$charset='',$dontEncodeHeader=false)	{
		...
		if (!$dontEncodeHeader)	{
			...
			foreach (explode(chr(10),$headers) as $line)	{	// Split the header in lines and convert each line separately
				...
				if (count($parts)==2)	{
					...
					$parts[1] = tx_div2007_div::encodeHeader($parts[1],$encoding,$charset);
			...
			$email = tx_div2007_div::encodeHeader($email,$encoding,$charset);		// Email address must not be encoded, but it could be appended by a name which should be so (e.g. "Kasper Sk�rh�j <kasperYYYY@typo3.com>")
			$subject = tx_div2007_div::encodeHeader($subject,$encoding,$charset);
		}
		...
		switch ((string)$encoding)	{
			...
			default:
				...
				$message=tx_div2007_div::quoted_printable($message);
			break;
		}
		...
		$headers = trim(implode(chr(10), tx_div2007_div::trimExplode(chr(10), $headers, true)));	// Make sure no empty lines are there.

		$ret = @mail($email, $subject, $message, $headers);
		if (!$ret)	{
			tx_div2007_div::sysLog('Mail to "'.$email.'" could not be sent (Subject: "'.$subject.'").', 'Core', 3);
		}
		return $ret;
	}
	...
	public static function encodeHeader($line,$enc='quoted-printable',$charset='iso-8859-1')	{
		...
		foreach ($matches as $part)	{
			...
			switch ((string)$enc)	{
				...
				case 'quoted-printable':
				default:
					$qpValue = tx_div2007_div::quoted_printable($part,1000);
					if ($part!=$qpValue)	{
			...
			foreach ($urlSplit as $c => &$v) {
				if ($c)	{
					...
					switch ((string)$urlmode)	{
						case 'all':
							$newURL = tx_div2007_div::makeRedirectUrl($newURL,0,$index_script_url);
						break;
						case '76':
							$newURL = tx_div2007_div::makeRedirectUrl($newURL,76,$index_script_url);
						break;
					}
	 ...
	 * @param	string		URL of "index script" - the prefix of the "?RDCT=..." parameter. If not supplyed it will default to tx_div2007_div::getIndpEnv('TYPO3_REQUEST_DIR').'index.php'
	...
	public static function makeRedirectUrl($inUrl,$l=0,$index_script_url='')	{
		if (strlen($inUrl)>$l)	{
			...
			$inUrl=($index_script_url ? $index_script_url : tx_div2007_div::getIndpEnv('TYPO3_REQUEST_DIR').'index.php').
				'?RDCT='.$md5;
		}
	...
	public static function initSysLog()	{
		...
		if (defined('TYPO3_cliMode') && TYPO3_cliMode)	{
			$TYPO3_CONF_VARS['SC_OPTIONS']['t3lib/class.tx_div2007_div.php']['systemLogHost'] = tx_div2007_div::getHostname($requestHost=FALSE).':'.PATH_site;
		}
			// for Web logging name is <protocol>://<request-hostame>/<site-path>
		else {
			$TYPO3_CONF_VARS['SC_OPTIONS']['t3lib/class.tx_div2007_div.php']['systemLogHost'] = tx_div2007_div::getIndpEnv('TYPO3_SITE_URL');
		}

			// init custom logging
		if (is_array($TYPO3_CONF_VARS['SC_OPTIONS']['t3lib/class.tx_div2007_div.php']['systemLog']))	{
			$params = array('initLog'=>TRUE);
			$fakeThis = FALSE;
			foreach ($TYPO3_CONF_VARS['SC_OPTIONS']['t3lib/class.tx_div2007_div.php']['systemLog'] as $hookMethod)	{
				tx_div2007_div::callUserFunction($hookMethod,$params,$fakeThis);
			}
		}
		...
		foreach (explode(';',$TYPO3_CONF_VARS['SYS']['systemLog'],2) as $log)	{
			...
			if ($type == 'syslog')	{
				...
				openlog($TYPO3_CONF_VARS['SC_OPTIONS']['t3lib/class.tx_div2007_div.php']['systemLogHost'], LOG_ODELAY, $facility);
			}
		}

		$TYPO3_CONF_VARS['SYS']['systemLogLevel'] = tx_div2007_div::intInRange($TYPO3_CONF_VARS['SYS']['systemLogLevel'],0,4);
		$TYPO3_CONF_VARS['SC_OPTIONS']['t3lib/class.tx_div2007_div.php']['systemLogInit'] = TRUE;
	}
	...
	public static function sysLog($msg, $extKey, $severity=0) {
		...
		$severity = tx_div2007_div::intInRange($severity,0,4);

			// is message worth logging?
		if (intval($TYPO3_CONF_VARS['SYS']['systemLogLevel']) > $severity)	return;

			// initialize logging
		if (!$TYPO3_CONF_VARS['SC_OPTIONS']['t3lib/class.tx_div2007_div.php']['systemLogInit'])	{
			tx_div2007_div::initSysLog();
		}
		...
		foreach (explode(';',$TYPO3_CONF_VARS['SYS']['systemLog'],2) as $log)	{
			...
			elseif ($type == 'mail')	{
				...
				mail($to, 'Warning - error in TYPO3 installation',
					'Host: '.$TYPO3_CONF_VARS['SC_OPTIONS']['t3lib/class.tx_div2007_div.php']['systemLogHost']."\n".
					'Extension: '.$extKey."\n".
					'Severity: '.$severity."\n".
					"\n".$msg,
					($from ? 'From: '.$from : '')
			...
			elseif ($type == 'error_log')	{
				error_log($TYPO3_CONF_VARS['SC_OPTIONS']['t3lib/class.tx_div2007_div.php']['systemLogHost'].$msgLine, 0);
			}
				// use the system log
			elseif ($type == 'syslog')	{
	...
	public static function devLog($msg, $extKey, $severity=0, $dataVar=FALSE)	{
		...
		if (is_array($TYPO3_CONF_VARS['SC_OPTIONS']['t3lib/class.tx_div2007_div.php']['devLog']))	{
			$params = array('msg'=>$msg, 'extKey'=>$extKey, 'severity'=>$severity, 'dataVar'=>$dataVar);
			$fakeThis = FALSE;
			foreach($TYPO3_CONF_VARS['SC_OPTIONS']['t3lib/class.tx_div2007_div.php']['devLog'] as $hookMethod)	{
				tx_div2007_div::callUserFunction($hookMethod,$params,$fakeThis);
			}
		}
	}
	...
	public static function deprecationLog($msg) {
		...
		$destination = PATH_typo3conf . '/deprecation_' . tx_div2007_div::shortMD5(PATH_site . $GLOBALS['TYPO3_CONF_VARS']['SYS']['encryptionKey']) . '.log';
		$file = @fopen($destination, 'a');
		if ($file) {
...
*/
	...
	public static function arrayToLogString(array $arr, $valueList=array(), $valueLength=20) {
		...
		if (!is_array($valueList))	{
			$valueList = tx_div2007_div::trimExplode(',', $valueList, 1);
		}
		$valListCnt = count($valueList);
		foreach ($arr as $key => $value)	{
			if (!$valListCnt || in_array($key, $valueList))	{
				$str .= (string)$key.trim(': '.tx_div2007_div::fixed_lgd_cs(str_replace("\n",'|',(string)$value), $valueLength)).'; ';
			}
		}
		return $str;
	}
	...
	public static function imageMagickCommand($command, $parameters, $path='')	{
		...
		if($command=='composite' && $switchCompositeParameters)	{	// Because of some weird incompatibilities between ImageMagick 4 and 6 (plus GraphicsMagick), it is needed to change the parameters order under some preconditions
			$paramsArr = tx_div2007_div::unQuoteFilenames($parameters);
