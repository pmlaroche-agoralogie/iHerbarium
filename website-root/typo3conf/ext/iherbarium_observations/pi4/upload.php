...
class UploadHandler
{
  ...
  function __construct($options=null) {
    $this->options = array(
			   ...
			   'param_addFilesToken' => 'addFilesToken',
			   'param_name' => 'files',
  ...
  private function handle_file_upload($uploaded_file, $name, $size, $type, $error) {
	...
	// DateTime
	$file->exifDateTime = NULL;
	if(isset($exif['DateTimeOriginal']))
	  $file->exifDateTime = $exif['DateTimeOriginal'];
  ...
  public function addFilesToken() {
    $addFilesToken = isset($_REQUEST['addFilesToken']) ? ($_REQUEST['addFilesToken']) : "";
		
    if(! preg_match('/token_\d+_\d+/', $addFilesToken) )
      $addFilesToken = "no_token";
			
    $this->options['addFilesToken'] = $addFilesToken;

    $tmpUploadDir = $addFilesToken . "/";


    // Main file.
    $this->options['upload_dir'] .= $tmpUploadDir;
    $this->options['upload_url'] .= $tmpUploadDir;

    if(! is_dir($this->options['upload_dir']))
      mkdir($this->options['upload_dir']);

    // File versions.
    foreach($this->options['image_versions'] as $image_version => $value) {
      $this->options['image_versions'][$image_version]['upload_dir'] .= $tmpUploadDir;
      $this->options['image_versions'][$image_version]['upload_url'] .= $tmpUploadDir;

      if(! is_dir($this->options['image_versions'][$image_version]['upload_dir']))
	mkdir($this->options['image_versions'][$image_version]['upload_dir']);
    }
